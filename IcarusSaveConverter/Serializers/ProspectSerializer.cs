...
{
	/// <summary>
	/// Splits and combines prospect saves
	/// </summary>
	internal static class ProspectSerializer
	{
		/// <summary>
		/// Split a prospect save
		/// </summary>
		internal static int SplitSave(ProspectSave prospect, string partsPath, bool useActorId, Logger logger)
		{
		...
		/// <summary>
		/// Returns whether the given directory appears to contain a split prospect save
		/// </summary>
		public static bool IsProspectParts(string path)
		{
			return File.Exists(Path.Combine(path, "ProspectInfo.json"))
				&& File.Exists(Path.Combine(path, "ProspectData.json"))
				&& Directory.Exists(Path.Combine(path, "Recorders"));
		}
	}
}
